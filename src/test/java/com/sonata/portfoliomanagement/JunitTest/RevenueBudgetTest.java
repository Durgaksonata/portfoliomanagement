...
class RevenueBudgetTest {
    ...
    @Captor
    private ArgumentCaptor<List<String>> classificationsCaptor;
    @BeforeEach
    public void setup() {
    ...
    public void getRevenueTest() {
        // Create mock data
        RevenueBudgetSummary mockRevenue = new RevenueBudgetSummary(21, "Vertical1", "Classification1", "DD1", "DM1", "Account1", "PM1", "Project1", 2024, "Q1", "April", 1000, 900, 100);

        // Mock repository behavior
        when(revenueRepo.findById(21)).thenReturn(Optional.of(mockRevenue));

        // Call the service method (or directly the repository if no service exists)
...
public void testGetClassificationByVertical() {
    ...
    List<RevenueBudgetSummary> mockRevenues = Arrays.asList(
            new RevenueBudgetSummary(1, "Vertical1", "Classification1", "DD1", "DM1", "Account1", "PM1", "Project1", 2024, "Q1", "April", 1000, 900, 100),
            new RevenueBudgetSummary(2, "Vertical1", "Classification2", "DD2", "DM2", "Account2", "PM2", "Project2", 2024, "Q1", "April", 1500, 1300, 200),
            new RevenueBudgetSummary(3, "Vertical2", "Classification1", "DD3", "DM3", "Account3", "PM3", "Project3", 2024, "Q1", "April", 1200, 1100, 100)
    );

    // Mock repository behavior
    when(revenueRepo.findByVerticalIn(Arrays.asList("Vertical1", "Vertical2"))).thenReturn(mockRevenues);
...
//    }
    ...
    @Test
    public void testGetDataByVerticalAndClassification() {
        // Mock request data
        RevenueDTO requestDTO = new RevenueDTO();
        requestDTO.setVerticalList(Arrays.asList("Vertical1", "Vertical2"));
        requestDTO.setClassificationList(Arrays.asList("Classification1", "Classification2"));

        // Mock data for revenue budget summaries
        List<RevenueBudgetSummary> mockDataList = Arrays.asList(
                new RevenueBudgetSummary(1, "Vertical1", "Classification1", "DD1", "DM1", "Account1", "PM1", "Project1", 2024, "Q1", "April", 1000, 900, 100),
                new RevenueBudgetSummary(2, "Vertical1", "Classification2", "DD2", "DM2", "Account2", "PM2", "Project2", 2024, "Q1", "April", 1500, 1300, 200),
                new RevenueBudgetSummary(3, "Vertical2", "Classification1", "DD3", "DM3", "Account3", "PM3", "Project3", 2024, "Q1", "April", 1200, 1100, 100)
        );
        ...
        // Mock repository behavior
        when(revenueRepo.findByVerticalInAndClassificationIn(
                requestDTO.getVerticalList(), requestDTO.getClassificationList())).thenReturn(mockDataList);

        // Call the method
        ResponseEntity<Set<Map<String, Object>>> responseEntity = controller.getDataByVerticalAndClassification(requestDTO);
        Set<Map<String, Object>> responseData = responseEntity.getBody();

        // Verify the result
        Set<Map<String, Object>> expectedData = new HashSet<>();
        for (RevenueBudgetSummary dataItem : mockDataList) {
        ...
        // Verify interactions
        verify(revenueRepo, times(1)).findByVerticalInAndClassificationIn(
                verticalsCaptor.capture(), classificationsCaptor.capture());
        assertEquals(requestDTO.getVerticalList(), verticalsCaptor.getValue());
        assertEquals(requestDTO.getClassificationList(), classificationsCaptor.getValue());
    }

    private boolean areAllCriteriaProvided(RevenueDTO requestDTO) {
        return requestDTO != null && requestDTO.getVerticalList() != null && !requestDTO.getVerticalList().isEmpty()
